...
# Configure Everything Claude Code (Multiversal)
...
- User says "configure multiversal", "install multiversal", "setup everything claude code", or similar
- User wants to selectively install skills or rules from this project
- User wants to verify or fix an existing Multiversal installation
- User wants to optimize installed skills or rules for their project

## Prerequisites

This skill must be accessible to Claude Code before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install multiversal@multiversal` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.claude/skills/configure-multiversal/SKILL.md`, then activate by saying "configure multiversal"

---

## Step 0: Clone Multiversal Repository

Before any installation, clone the latest Multiversal source to `/tmp`:
...
If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing Multiversal clone.
...
```
Question: "Where should Multiversal components be installed?"
Options:
...
`claude-api` is an Anthropic canonical skill. Install it from [`anthropics/skills`](https://github.com/anthropics/skills) when you want the official Claude API workflow instead of an Multiversal-bundled copy.
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source Multiversal repository (`$ECC_ROOT/`).
...
```
## Multiversal Installation Complete
