---
name: multiversal-guide
description: Guide users through Multiversal's current agents, skills, commands, hooks, rules, install profiles, and project onboarding by reading the live repository surface before answering.
origin: community
---
...
Answer from current files, not memory. Multiversal changes quickly, so hard-coded catalog counts, feature lists, and install instructions go stale.

When the Multiversal repository is available, inspect the relevant files before giving a concrete answer:
...
Use `/project-init` when the user wants Multiversal configured for a target repo. The expected sequence is:
