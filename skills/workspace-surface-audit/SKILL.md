...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value Multiversal-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
origin: Multiversal
---
...
This is the Multiversal-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against Multiversal-native coverage
...
- Prefer Multiversal-native workflows over generic "install another plugin" advice when Multiversal can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in Multiversal
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent Multiversal workspaces
...
- existing Multiversal skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but Multiversal lacks a billing-operator skill"
- "Google Drive is connected, but there is no Multiversal-native Google Workspace operator workflow"
...
2. whether Multiversal already has parity
3. whether Multiversal only has primitives
4. whether Multiversal is missing the workflow entirely

### Phase 3: Turn Gaps Into Multiversal Decisions

For every real gap, recommend the correct Multiversal-native shape:

| Gap Type | Preferred Multiversal Shape |
|----------|---------------------|
...
2. **Parity**
   - where Multiversal already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but Multiversal lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete Multiversal-native additions, ordered by impact
...
- If Multiversal already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what Multiversal should own next.
