...
This skill ships with Everything Claude Code. No separate installation is needed when Multiversal is installed.

### Full Multiversal install

If you are working from the Multiversal repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the Multiversal diff before updating:
...
If you are vendoring only this skill outside the full Multiversal install, copy the reviewed file from the Multiversal repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed Multiversal commit rather than running `git pull`.
